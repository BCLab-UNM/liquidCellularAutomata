...
# @TODO add system arguments for sleep time(speed), radius, colors, color balance, walk angles/type,rng seed,
#  #agents, ect.
#  @TODO create an agent and agents/lca class and main
# @TODO add collions, going dark(change network) duplex
class Agent:
    def __init__(self, node_degree=2):
        self.turt = turtle.Turtle()
        self.seed = None 
        self.radius = None
        self.node_degree = node_degree
        self.group_id = 0
        self.duplex = None # like does it get influenced or is an influencer?
        time.sleep(.1)
        self.orignal_pose = (self.turt.xcor(), self.turt.ycor())
...
#args = parser.parse_args(["--agents","10","--vis"])
args = vars(parser.parse_args(["--agents","100"]))
print(args)
...
screen.update()
print("Startings consensus: ", consensus(agents))
...
while not consensus_reached(agents):
  ...
  for agent in agents:
    ...
    if abs(agent.turt.ycor()) > half_height or abs(agent.turt.xcor()) > half_width:
      ...
      agent.turt.right(
        random.randint(160, 200))  # 180 would perfect bounce but at the corner would be bad could get stuck oscillating
    ...
    agent.turt.color("black", consensus(agent.within_range(agents)))
  screen.update()
